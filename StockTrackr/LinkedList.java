public class LinkedList
{
    ...
    public Object removeNode1(String remove) throws EmptyListException
    {
        if(isEmpty())
            throw new EmptyListException();
         
        if (((IngredientStock) head.data).getStockName().equals(remove)) 
            return removeFromFront();
        
        Node prev = head;
        Node curr = head.link;
        
        while (curr != null && !((IngredientStock) curr.data).getStockName().equals(remove)) 
        {
            prev = curr;
            curr = curr.link;
        }
